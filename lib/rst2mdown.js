...
(function () {
	...
	var markdown = function (tokens) {
		...
		function space(up) {
			return blockDepth ? repeatString(' > ', blockDepth) : '' +
				(indent ? repeatString('  ', indent) : '') +
				(lists.length ? (lists[lists.length - 1].ordered ? (repeatString('   ', up ? (lists.length - 1) : lists.length)) :
					(repeatString('  ', up ? (lists.length - 1) : lists.length))) : '');
		}

...
var markdown = function(tokens){
	...
	function space(up){
		return blockDepth ? " > ".repeat(blockDepth) : "" +
			lists.length ? "  ".repeat(up ? (lists.length - 1) : lists.length) : "";
	}
	...
	tokens.forEach(function(token){
		switch(token.type){
			...
			case "list_item_start":
				docadd(lists[lists.length - 1] ? olcount + ". " : "- ", true);
				noSpace = true;
				olcount++;
				break;
			case "list_item_end":
